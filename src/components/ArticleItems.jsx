...
function ArticleItems({
  ...
  return (
    <div className="md:w-fit  md:h-[90vh] md:overflow-y-auto w-full overflow-x-auto md:pr-4 items-scrollbar">
      ...
      <ul className="text-[#808080]  md:space-y-7 space-x-7 md:space-x-0 bg-transparent flex flex-row md:flex-col mb-2 ">
        {data?.map((el, i) => {
          if (
            i !== coverArticleIndex &&
            ...
            return (
              <li
                className="flex min-w-fit space-x-5 md:space-x-0  scroll-item items-center cursor-pointer transition-all bg-[#1f1e1ebe] rounded-md pr-3"
                key={el.data[0].
                  nasa_id}
              ...
              >
                <img
                  src={el.links[0].href}
                  alt="article-cover"
                  className="w-28 h-28 md:w-32 md:h-32 object-cover md:mr-3 rounded-md"
                />
