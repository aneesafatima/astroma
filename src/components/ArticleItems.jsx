...
function ArticleItems({ data, coverArticleIndex, setCoverArticleIndex, setCoverArticle }) {
  ...
  return (<div className='md:w-fit  md:h-screen md:overflow-hidden w-full overflow-x-auto md:pr-4 '>
    {/* <div className=" "> */}
      <ul className='text-[#808080]  md:space-y-7 space-x-7 md:space-x-0 bg-transparent flex flex-row md:flex-col '>
        {
          data?.map((el, i) => {
            if (i !== coverArticleIndex && !uniqueTitlesSet.has(el.data[0].title)) {
              ...
              return (
                ...
                <li className='flex min-w-fit md:space-x-0 space-x-5 scroll-item items-center cursor-pointer transition-all bg-[#1f1e1ebe] rounded-md pr-3' key={i} onClick={() => changeCoverArticle(i)}>
                  <img src={el.links[0].href} alt="article-cover" className='w-28 h-28 md:w-32 md:h-32 object-cover md:mr-3 rounded-md' />
                  <div className=' flex flex-col text-xs space-y-3'>
                    ...
                    <h2 className='text-white text-sm md:text-lg '>{el.data[0].title.substring(0, 33)}...</h2>
