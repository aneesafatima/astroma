        ...
        <div className="flex pt-10 flex-col spaxe-x-5 md:flex-row flex-wrap md:flex-nowrap space-y-6 ">
          <div className="flex mr-10 pr-4 space-y-6 justify-center md:justify-normal items-start flex-col flex-wrap w-full md:w-[70%]">
            <div className="w-full flex flex-col md:flex-row items-center space-x-3 space-y-6">
              ...
              <img
                src={coverArticle?.links[0]?.href}
                className="w-[390px] h-72 mx-10 object-cover"
                alt="article cover"
              />
            </div>
            <div className="w-full space-y-5">
              ...
              <ul className="text-[#808080] flex text-xs pr-8" id="tag">
                <li className="border-r-[1px] border-[#808080] px-5 flex space-x-1">
                  {coverArticle?.data[0]?.keywords?.map((el, i) => {
                    if (i < 2)
                      return <li key={i}>{el + `${i !== 1 ? "," : ""}`}</li>;
                  }) ?? "Space and Universe"}
                </li>
                <li className="border-r-[1px] border-[#808080] px-5 ">
                  {coverArticle?.data[0]?.photographer?.split("/")[1] ??
                    "unknown"}
                </li>
                <li className="px-5 ">
                  {new Date(
                    coverArticle?.data[0]?.date_created
                  ).toLocaleDateString()}
                </li>
              </ul>
            </div>
          ...
          <ArticleItems
            ...
            coverArticleIndex={coverArticleIndex}
            setCoverArticleIndex={setCoverArticleIndex}
            setCoverArticle={setCoverArticle}
          />
        </div>
      )}
    </div>
