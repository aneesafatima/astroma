...
function HomePage() {
 ...
 const [showAbout, setShowAbout] = useState(false);
  ...
  return (<>
    ...
    <ul className={`flex flex-col items-center space-y-10 xs:space-y-0 xs:flex-row justify-center uppercase xs:space-x-7 sm:space-x-10 text-xs sm:text-sm font-light  opacity-${visible ? '1' : '0'} transition-opacity duration-700  ease-in`}>
    <li className=''>
  <Link to="/" className='hover:text-gray-400 mt-8'>Home</Link>
    </li>
    <li>
  <Link to="/planetary-hub" className='hover:text-gray-400'>PLanetary hub</Link>
    </li>
    <li>
  <Link to="/stargazing-guide" className='hover:text-gray-400'>Stargazing</Link>
    </li>
    <li>
  <Link to="/cosmic-gallery"  className='hover:text-gray-400'>Cosmic Gallery</Link>
    </li>
  </ul>
  ...
  <div className={`bg-transparent flex flex-wrap   ${showAbout ? "w-full opacity-1" : "w-0 opacity-0"} transition-all duration-1000  ease-in-out `}><img src={astronauts} alt="Astronauts" className='w-full object-contain  sm:w-[50%] mr-8'/>
