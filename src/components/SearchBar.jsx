...
[...document.getElementsByClassName("categories")].forEach(el => el.classList.remove("active"));
  ...
  return ( 
    <div className='pt-7 flex justify-evenly flex-wrap space-y-7 md:space-y-0 text-[#808080] text-xs font-PtSans tracking-widest items-center'>
     
    <ul className=' flex justify-center space-x-6 md:space-x-10 '>
        <li className='hover:text-white cursor-pointer categories' onClick={(e) => setSearchCategory("general", e)}>All</li>
...
<div className='border-b-[1px] flex items-center  border-[#808080] pb-2 px-3 '><input type="text" placeholder={`search to explore`} className='bg-transparent text-[#808080] outline-0 md:w-52 px-3' id='search-input' ></input> <CiSearch onClick={setSearchValue} className='cursor-pointer hover:scale-[.92]'/></div>        
    </div>
  )
}
