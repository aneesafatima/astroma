...
function PlanetsData({showPlanetInfo, planets, currentPlanet, data, planetDef}) {
  ...
  return (
    <div className={` ${!showPlanetInfo ? "opacity-0 h-0 duration-0" : " h-fit lg:h-[90vh] flex space-x-5 lg:justify-evenly lg:items-center w-full text-sm uppercase opacity-100 transition-all duration-1000"} `} style={{color: planets[currentPlanet].textColor}} id="planets-info">
    <div className={``} id="general-info">
      ...
      <p className={`w-[50%] hidden lg:block text-sm tracking-wide leading-relaxed lowercase`}>{planetDef.extract}</p>
